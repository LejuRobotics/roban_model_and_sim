...
color_mask = [
    ...
    lambda img: cv.inRange(cv.cvtColor(img,cv.COLOR_BGR2HSV), np.array([0, 0, 50]), np.array([30,30,125]))
]
...
def main():
    ...
    #-----------------------------------------------------------------------------------------------------------------------------
    #强行走一段
    linearTrajMoveTo(torso=[[0,0,0.14]], right_arm=[(0, -0.15, -0.16)])
    walkingInit()
    rospy.sleep(0.5)
    pause = threading.Event()
    ...
    if(color_type==0):
        headCtrl(yaw=20,pitch=15, wait=False)
        for i in range(3):
            sendGaitCommand(0,0,-30)
        for i in range(6):
            sendGaitCommand(0.10,0,0)
        for i in range(6):
            sendGaitCommand(0.07,0,10)
        waitForWalkingDone()
        while(not stable):
            print('等不晃了')
            rospy.sleep(0.2)
        ...
        if(abs(walk_dist[1])>0.05):
            theta = 0.8*degrees(atan2(walk_dist[1], walk_dist[0]))
            sendGaitCommand(0,0, theta)
            rospy.logerr('转%f deg', theta)
    elif(color_type==1):
        headCtrl(yaw=20,pitch=15, wait=False)
        for i in range(3):
            sendGaitCommand(0,0,-30)
        ...
        headCtrl(yaw=-30,pitch=15, wait=False)
        for i in range(3):
            sendGaitCommand(0,0,-30)
        for i in range(4):
            sendGaitCommand(0.10,0,0)
